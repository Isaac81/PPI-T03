...
const db = new sqlite3.Database('./tareas.db', (err) => {
    ...
    app.post('/tareas', (req, res) => {
        const { descripcion } = req.body.text;
    ...
    app.patch('/tareas/:id', (req, res) => {
        const { id } = req.params;
        const inputData = [req.body.completada, id];
        const query = `UPDATE tareas SET completada=? WHERE id = ?`;
        db.run(query, inputData, function (err) {
            if (err) {
                res.status(400).json({ error: err.message });
                return;
            }
            res.json({ message: 'Tarea parcheada correctamente' });
        });
    });